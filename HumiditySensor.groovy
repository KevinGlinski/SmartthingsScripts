...
Update 6: Aug 21, 2018
Changes to primarily use it as a humidity sensor
/*
...
metadata {
        ...
        multiAttributeTile(name:"motion", type: "generic", width: 6, height: 4){
            tileAttribute ("device.primaryDisplay", key: "PRIMARY_CONTROL") {
                attributeState "",  label: '${currentValue}%', backgroundColors:[
                   ...
                   [value: 59, color: "#c5f233"],
                   [value: 66, color: "#f1d801"],
                   [value: 74, color: "#d04e00"],
    ...
    preferences {
        input "primaryDisplayType", "enum", options: ["Motion", "Temperature", "Humidity"], title: "Primary Display", defaultValue: "Motion",
...
def updateDisplay(String source) {
    ...
    switch (primaryDisplayType) {
       ...
       case "Humidity": 
           if(source == "multi") {
               evt += createEvent([name:"primaryDisplay", value: state.humidity, displayed:false])
           }
           evt += createEvent([name: "sensorlevels", value: "${state.motionText}   Temperature:${state.temp}°F   Light:${state.illuminance}"])
           break
    }
    return evt
}
